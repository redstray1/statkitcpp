        ...
        for (size_t right = 0; right < max_index_right; right++)  {
            ...
            for (size_t k = 0; k < shape[dim]; k++) {
                ...
                if (!started) {
                    ...
                    out[out_index] = std::max(out[out_index], data[src_index]);
                }
            }
        }
    }
...
void max(const Storage& data, //NOLINT
         ScalarType dtype,
         const std::vector<size_t>& shape,
         const std::vector<size_t>& strides,
         int dim,
    ...
    case (ScalarType::name): {\
        ...
        max_<T>(p, ItemSize(dtype), data.GetNbytes(), dim, shape, strides, o, out.GetNbytes()); \
        break; \
    }
    switch (dtype) {
